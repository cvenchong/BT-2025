                ...
                .addEventListener("submit", (event) => {
                  ...
                  hostedFields
                    ...
                    .then(async (response) => {
                      if (response.status === 201) {
                        ...
                        if (response.status === 207) {
                          ...
                          console.log(
                            `Transaction with ID ${body.id} successfully created and settled but vaulting failed`
                          );
                        }
                    ...
                    .then((data) => {
                      ...
                      return [data.transactionId, data.customerId];
                    })
                    .then(([transactionId, customerId]) => {
                      console.log(
                        `Transaction with ID ${transactionId} successfully created, settled and vaulted with customer ID ${customerId}`
                      );
                      document.getElementById(
                        "feedback"
                      ).innerHTML = `Transaction with ID ${transactionId} successfully created, settled and vaulted with customer ID ${customerId}`;
                      document.getElementById("error").innerHTML = "";
                      ...
                      useLink.setAttribute(
                        "href",
                        `../vault-card-redirect/index.html?customer-id=${customerId}`
                      );
                    })
                    .catch((error) => {
