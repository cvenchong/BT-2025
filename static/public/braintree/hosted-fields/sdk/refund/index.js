...
function refundTransaction() {
  console.log("Received refund transaction request");
  ...
  console.log(`Attempting to refund transaction with ID ${transactionId}`);
  fetch(`${BASE_URL}/braintree/sdk/transaction/refund`, {
    method: "POST",
    headers: {
    ...
    .then(async (response) => {
      if (response.status === 204) {
        console.log(
          `Transaction with ID ${transactionId} successfully refunded`
        );
        document.getElementById(
          "feedback"
        ).innerHTML = `Transaction with ID ${transactionId} successfully refunded`;
        document.getElementById("error").innerHTML = "";
...
function main() {
  ...
  document
    .getElementById("refund-button")
    .addEventListener("click", refundTransaction);
                ...
                .addEventListener("submit", (event) => {
                  ...
                  hostedFields
                    ...
                    .then((transactionId) => {
                      ...
                      let refundButton =
                        document.getElementById("refund-button");
                      refundButton.hidden = false;
                      refundButton.setAttribute(
                        "data-transaction-id",
                        transactionId
                      );
                    })
