...
function updateTotal() {
  ...
  console.log("Getting messaging information");
  fetch(`${BASE_URL}/paypal/rest/credit/messaging-information`, {
    method: "POST",
    headers: {
    ...
    .then((data) => {
      if ((data?.message ?? null) === null) {
        throw new Error("No message with associated messaging information");
      }
      if ((data?.disclaimer ?? null) === null) {
        throw new Error("No disclaimer with associated messaging information");
      }
      if ((data?.clickUrl ?? null) === null) {
        throw new Error("No clickUrl with associated messaging information");
      }
      if ((data?.impressionUrl ?? null) === null) {
        throw new Error(
          "No impressionUrl with associated messaging information"
        );
      }

      return [data.message, data.disclaimer, data.clickUrl, data.impressionUrl];
    })
    .then(([message, disclaimer, clickUrl, impressionUrl]) => {
      document.getElementById("message").innerHTML = message;
      document.getElementById("disclaimer-message").innerHTML = disclaimer;
      document.getElementById("disclaimer-link").setAttribute("href", clickUrl);
      document.getElementById("impression").setAttribute("src", impressionUrl);
    })
    .catch((error) => {
      console.error(error);
