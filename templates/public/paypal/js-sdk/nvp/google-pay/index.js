...
async function onApprove(paymentData) {
  ...
  if ((paymentData?.paymentMethodData ?? null) === null) {
    console.error("No payment method data with associated payment data");
    document.getElementById("feedback").innerHTML = "Error approving order";
    document.getElementById("error").innerHTML =
      "No payment method data with associated payment data";
    return;
  }

  console.log("Attempting to create order");
  const id = await fetch(`${BASE_URL}/paypal/nvp/expressCheckout/new`, {
    method: "POST",
    headers: {
  ...
  return await paypal
    ...
    .then(async (response) => {
      if (response?.status === "APPROVED") {
        // need to call GetExpressCheckoutDetails to get payer ID from response
        console.log("Getting payment details");
        return await fetch(`${BASE_URL}/paypal/nvp/expressCheckout/get`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          ...
          .then((data) => {
            if ((data?.id ?? null) === null) {
              throw new Error("No payer ID with associated approved order");
            }

            return data.payerId;
          })
          .then((payerId) => {
            console.log(`Received payer ID ${payerId}`);
            document.getElementById(
              "feedback"
            ).innerHTML = `Received payer ID ${payerId}`;
            document.getElementById("error").innerHTML = "";

            console.log("Attempting to capture order");
            return fetch(`${BASE_URL}/paypal/nvp/expressCheckout/complete`, {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
